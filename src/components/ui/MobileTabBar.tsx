...
import { Home, Car, Target, SignpostBig, BarChart3 } from "lucide-react";
...
export function MobileTabBar() {
  ...
  const tabs = [
    ...
    { href: "/bien-bao", label: "Biển báo", icon: SignpostBig, active: path.startsWith("/bien-bao") },
    ...
    >
      ...
      <ul className="grid grid-cols-5">
        {tabs.map(({ href, label, icon: Icon, active }) => (
          <li key={label}>
            <Link href={href} className="flex flex-col items-center gap-1 py-2 text-[10.5px] font-semibold" aria-current={active ? "page" : undefined}>
              <span
                className={clsx(
